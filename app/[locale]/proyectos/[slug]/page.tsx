...
export default async function ProjectDetailPage({
    ...
    const components = {
        h2: (props: React.ComponentPropsWithoutRef<'h2'>) => <h2 {...props} className="text-3xl font-serif mt-12 mb-6" />,
        p: (props: React.ComponentPropsWithoutRef<'p'>) => <p {...props} className="mb-6 leading-relaxed opacity-80 text-lg" />,
        ul: (props: React.ComponentPropsWithoutRef<'ul'>) => <ul {...props} className="list-disc pl-6 mb-6 space-y-2" />,
        li: (props: React.ComponentPropsWithoutRef<'li'>) => <li {...props} className="opacity-80" />,
    };
                                        ...
                                        <div className="flex flex-wrap gap-3">
                                            {project.tools.map(tool => {
                                                ...
                                                return (
                                                    <div
                                                        key={tool}
                                                        className="group relative flex items-center justify-center bg-background/80 backdrop-blur-md border border-white/10 p-3 rounded-2xl w-14 h-14 transition-all duration-300 hover:scale-105 hover:bg-white/5 hover:border-white/20"
                                                        title={tool}
                                                    >
                                                        {logo ? (
                                                            <Image src={logo} alt={tool} fill className="object-contain p-3" />
                                                        ) : (
                                                            <span className="text-[10px] text-center leading-tight opacity-70 cursor-default select-none">{tool}</span>
                                                        )}
                                                    </div>
                                                )
                                            })}
